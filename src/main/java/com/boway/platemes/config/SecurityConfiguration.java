package com.boway.platemes.config;

import com.boway.platemes.service.UserDetailsServiceImpl;
import com.boway.platemes.util.JwtAuthenticationFilter;
import com.boway.platemes.util.JwtAuthorizationFilter;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // 开启跨域
        http.cors().and()// security 默认 csrf 是开启的，我们使用了 token ，这个也没有什么必要了
                .csrf()
                .disable()
                .authorizeRequests()
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        /*security  默认鉴权完成后会把密码抹除，但是这里我们使用用户的密码来作为
            JWT 的生成密钥，
        如果被抹除了，在对   JWT  进行签名的时候就拿不到用户密码了
        故此处关闭了自动抹除密码。*/
